...
function parseResponseSheet(html) {
  ...
  // Pattern observed in real sheets: <td class="bold">Question ID</td><td></td><td>[ID]</td>
  // <td class="bold">Chosen Option</td><td></td><td>[optionId or --]</td>
  const questionIdRegex = /<td class="bold">Question ID<\/td><td><\/td><td>(\d+)<\/td>/gi;
  const chosenOptionRegex = /<td class="bold">Chosen Option<\/td><td><\/td><td>(\d+|--)<\/td>/gi;
  ...
  if (questionIds.length !== chosenOptions.length) {
    throw new Error(`Parsing mismatch: found ${questionIds.length} questions but ${chosenOptions.length} chosen options.`);
  }
  ...
  for (let i = 0; i < questionIds.length; i++) {
    ...
    questions.push({
      ...
      chosenOptionId: chosen === '--' ? null : chosen
    });
  }
...
function calculateScore(parsedQuestions, answerKeyMap) {
  ...
  for (const { questionId, chosenOptionId } of parsedQuestions) {
    ...
    if (!chosenOptionId) {
      ...
      if (questionType === 'mcq') score -= 1;
    }

    analysis.push({ questionId, chosenOptionId, correctOptionId, questionType, status });
  }
...
function subjectBreakdown(analysis, answerKeyRows) {
  ...
  // But since question IDs are NTA-assigned, we rely on subject column in answerkeys
  const subjectMap = {};
  for (const row of answerKeyRows) {
    if (row.subject) {
      subjectMap[row.questionid] = row.subject;
    }
  }
  ...
  for (const q of analysis) {
    ...
    if (!subjects[sub]) {
      subjects[sub] = { correct: 0, wrong: 0, unattempted: 0, score: 0 };
    }

    subjects[sub][q.status]++;
    if (q.status === 'correct') {
      subjects[sub].score += 4;
    } else if (q.status === 'wrong' && q.questionType === 'mcq') {
      subjects[sub].score -= 1;
    }
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: answerKeyRows, error: dbError } = await supabase
      .from('answerkeys')
      .select('questionid, correctoptionid, questiontype, subject')
      .eq('examdate', examDate)
      .eq('shift', shift);

    if (dbError) {
      return res.status(500).json({
        error: 'Database error fetching answer key.',
